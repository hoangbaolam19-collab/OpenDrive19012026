...
proxies_dic = {
    "http": "136.131.63.121:8082",
    "https": "136.131.63.121:8082",
}
...
def make_signature_hmac(http_method, parameter, uri, secret):
    ...
    params = urllib.parse.urlencode(
        sorted(parameter.items(), key=lambda x: x[0])
    )
    # URIを作成
    ...
    # oauth_signature
    signature = hmac.new(
        secret.encode(), base_string.encode(), hashlib.sha1
    ).digest()
    # base64エンコードを行う
...
def drive_route_multi(latlon):
    # Oauthのパラメータを設定
    http_method = "get"
    parameter = {
        "if_clientid": "JSZb5d4716d6b90|4rXuF",  # クライアントキー
        "if_auth_type": "oauth",
        "oauth_consumer_key": "JSZb5d4716d6b90|4rXuF",  # クライアントキー
    ...
    uri = "https://core.its-mo.com/zmaps/api/apicore/core/v1_0/road/latlon/drive"  # 使用するAPI
    secret = "KN0v34gU3KkzvDiiNqeD-bK5Ypc"  # キーに紐づく秘密鍵
...
def drive_route3(latlon, type):
    ...
    parameter = {
        ...
        "searchType": str(type),
        "searchparam": "0",
        "from": str(latlon[0]) + "," + str(latlon[1]),
        "to": str(latlon[2]) + "," + str(latlon[3]),
        "mpoints": "",
        "fromtype": "highway",
        "totype": "highway",
        "mpointstype": "highway",
    ...
    uri = "https://core.its-mo.com/zmaps/api/apicore/core/v1_0/route3/drive"  # 使用するAPI
    secret = "KN0v34gU3KkzvDiiNqeD-bK5Ypc"  # キーに紐づく秘密鍵
    make_signature_hmac(http_method, parameter, uri, secret)
    url = uri + "?" + urllib.parse.urlencode(parameter)
    req = urllib.request.Request(url)
