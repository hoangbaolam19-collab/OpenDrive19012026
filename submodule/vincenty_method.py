#参考URL：https://qiita.com/r-fuji/items/99ca549b963cedc106ab
# -*- coding: utf-8 -*-
from math import *
...
def coord2XY(lat1, lon1, lat2, lon2):
    # If points identical, return zeros (same behavior as vincenty_inverse)
    if isclose(lat1, lat2) and isclose(lon1, lon2):
        return 0.0, 0.0, 0.0, 0.0

    # Default to Zone VIII (6676) for backward compatibility
    src_srs, dst_srs, zone_epsg = get_coordinate_systems(location_prefecture=None, lat=None, lon=None, default_zone=6671)
    transformer = Transformer.from_crs(src_srs, dst_srs)

    # Use the project's projection (EPSG:4612 -> EPSG:6676) to get planar coordinates
    # Follow LinkData convention: pass (lat, lon) into transform and then swap returned
    # tuple elements as LinkData does (node_xy[1], node_xy[0]).
    ...
    # azimuth measured clockwise from North (degrees)
    azim_rad = atan2(dx, dy)
    azim = degrees(azim_rad)
    if azim < 0:
        azim += 360.0

    return dx, dy, dist, azim
